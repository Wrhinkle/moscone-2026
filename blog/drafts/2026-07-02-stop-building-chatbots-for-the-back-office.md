...
The answer shaped a strong opinion: the useful form of back-office AI is not conversational. It's an **evaluator** — messy operational context in; structured state, missing-item detection, and human-approvable next actions out. Chat is an interface mismatch for work that has a checklist.

## Why a chatbot doesn't fix this

The default move — and, from what I can see, still the default shape of most small-business "AI adoption" — is a chatbot bolted onto the CRM. Ask your data questions in natural language. It demos well and it fails at this job for reasons that are structural, not fixable with a better model.

**Chat is pull-based; the failure mode is that nobody pulled.** A chatbot answers questions someone thinks to ask. The Foster packet goes to production precisely because no one thought to ask "is the NOC recorded?" — the coordinator believed the packet was complete. An interface whose value depends on the operator already suspecting the gap is an interface that misses the gaps nobody suspects. The audit has to run unprompted, over everything, the same way every time.

**Chat output is prose; operational state needs to be diffable.** If you ask a chatbot "is the Foster job ready?", you get paragraphs. Paragraphs can't be compared across the forty other jobs in the pipeline, can't be regression-tested when you change the prompt, and can't drive a downstream tool call. A checklist evaluation — every required item resolved to `present | missing | unclear`, with an evidence snippet — is a data structure. You can sort on it, alert on it, and hold it to a standard.

**Conversation has no completion semantics.** A chat session is done when the human stops typing. A packet audit is done when every required item has a status and every blocking gap has an owner. That's the property that makes the output trustworthy: it is *exhaustive by construction*, not exhaustive because the model happened to be thorough today.

None of this is an argument that language models don't belong in the loop. It's an argument about where they belong: extraction and drafting, not interface and not verdict.

## The evaluator pattern

The pattern, mechanically. Five stages, one direction of data flow, ending at a human.
...
2. **Gap detection.** Diff the extracted state against a hardcoded required-item checklist. Every item resolves to a tri-state: `present`, `missing`, or `unclear`. The design rule that matters most: **prefer `unclear` over a false `present`.** Weak evidence becomes a follow-up question, never a silent pass.
3. **Risk classification.** Each checklist item carries a `blocking` flag, set by a human who knows the domain, in code — not inferred per-run by the model. Verdict semantics are then trivial and deterministic: `NOT READY` if any blocking item is missing *or unclear*; `READY WITH RISKS` if blocking items are all present but non-blocking ones aren't; `READY` only if everything required is present.
4. **Next actions, routed by owner.** Every gap becomes a task in one of two lanes — office follow-up (record the NOC, chase the HOA letter, lock the color) or production concerns (crew scope, materials, access). A gap without an owner is a gap that survives.
...
Honest status: the version that exists today is a local prototype whose extraction layer is keyword and pattern matching with the manual override as backstop; the LLM extraction pass — reading free-text communication logs the way a person does, feeding the same deterministic checklist — is the specified next slot in the design, along with a public repo and an obfuscated real packet as the demo fixture. The checklist authority, verdict semantics, tri-state statuses, lanes, and copy-ready notes are all running. Nothing about the pattern waits on the model getting smarter.

## The parsing layer is a commodity; the checklist is not

The expo floor made one thing unambiguous: turning ugly documents into structured JSON is now a product category with real capital and real customers, at every altitude of the stack.
...
Three papers, one finding: the reliability gains come from structure around the model — spec construction, enforced plans, typed checkpoints — not from the model itself. The evaluator pattern is that finding, applied to operations.
...
What I don't have yet is proof of quality at the standard I'm arguing for. An evaluator that ships on vibes is a chatbot with better formatting. The next build is the eval harness: twenty obfuscated packets with known verdicts, a scoring script, a regression run on every checklist or prompt change — the golden-set discipline the extraction vendors sell at enterprise scale, sized down to one operator and an afternoon. The open question I keep turning over is where the ceiling is: gap detection is clearly the evaluator's job, and releasing the job to production is clearly the human's — but risk *scoring*, the middle layer that says this missing item matters more than that one, sits uncomfortably between them. I know how I'd test it. Twenty packets at a time.
