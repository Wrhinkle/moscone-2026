...
Venture firms, accelerators/communities, media, universities, foundations, and government bodies sponsoring AIEWF 2026. Nothing in this folder is a tool you'd wire into an agent stack — the category exists because the agent ecosystem runs on more than code: someone funds the startups (VCs, PE, philanthropy), someone trains and connects the practitioners (media, communities, universities), and someone regulates the money (government). If you're a founder scoping investors, an engineer looking for learning channels, or anyone tracking where institutional capital thinks agents are headed, this is the folder.
...
1. **[UC Berkeley](uc-berkeley.md)** — the only entry whose output you might actually run in production: vLLM (serving), Gorilla/BFCL (tool-calling benchmarks), Chatbot Arena (evals). Foundational to several other areas of this repo.
2. **[Untapped Capital](untapped-capital.md)** — the most agent-native investor here: co-founded by Yohei Nakajima, creator of BabyAGI, one of the earliest autonomous-agent open-source projects. Small fund (~$15M), outsized relevance.
3. **[Amplify Partners](amplify-partners.md)** — the clearest portfolio-as-map: LangChain, Modal, Temporal, Datadog. If you want to know which infra layers institutional capital believes in, read Amplify's bets, then cross-check with [Gradient](gradient.md) ($220M AI-only seed fund, freshly spun out of Google).
...
| [Gates Foundation](gates-foundation.md) | World's largest private philanthropy funding AI for global health | Supporting | Endowed foundation — grants, not equity | $200M/4-year Anthropic partnership (May 2026): grant funding, API credits, shared tools for health/education/agriculture |
...
All 11 profiles expected in this category are present — none missing or re-filed elsewhere.
...
- **What happens to DFPI's VC reporting rules?** Enforcement of the founder-diversity disclosure program was suspended in March 2026 pending rulemaking — the compliance burden on every fund in this folder with a California nexus is currently undefined.
- **Overlap signal untested:** Acrew backs fellow sponsor Veris AI; Amplify backs LangChain; Carlyle uses LlamaIndex tooling. Nobody has yet mapped sponsor-to-sponsor investment edges across this repo — a small graph exercise that would show which "community" ties are actually cap-table ties.
