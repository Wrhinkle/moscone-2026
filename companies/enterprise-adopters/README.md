...
Large enterprises, consumer platforms, and institutions that show up at AIEWF 2026 as **AI adopters and hosts, not AI-tooling vendors**. Nothing in this folder is a product you'd `pip install` — these are the demand side of the agent economy: companies retrofitting agents onto decades-old marketplaces, rebuilding SDLCs around coding agents, exposing their product surfaces to other people's agents via MCP, or shipping LLM features to hundreds of millions of consumers. The real-world problem this category answers is the one every builder eventually faces: *does any of this agent stuff survive contact with production, compliance, and an existing business?* These profiles are the evidence base — deployment patterns, published architectures, and hard(ish) business metrics from organizations that had to make it work rather than sell it.

## How to read this category

Start with the three most load-bearing profiles:

- **[Optiver](optiver.md)** — the most concrete public account of an enterprise redesigning its SDLC *around* coding agents (75% time-to-market reduction claimed), plus a sober LLM-vs-intern trading evaluation.
- **[DoorDash](doordash.md)** — the best-documented multi-agent reference architecture in the folder: MCP tool surface, deep-agent hierarchies, A2A adoption, all in public engineering posts.
...
| [Block](block.md) | Fintech that open-sourced goose, an Apache-2.0 coding agent | Supporting | Public (NYSE: XYZ) | goose is LLM-agnostic, MCP-native, and adopted externally (Databricks, university labs) |
...
| [Indeed](indeed.md) | Job-search incumbent shipping agents on both marketplace sides | Supporting | Recruit Holdings subsidiary (acquired 2012, ~$1B) | Claims Sourcing Assistant applicants are 2.9x more likely to be hired, ~6 days faster |
...
| [LinkedIn](linkedin.md) | Microsoft-owned network running Hiring Assistant on a purpose-built agent platform | Supporting | Microsoft subsidiary (acquired 2016, $26.2B) | Reports 81% fewer profiles reviewed per qualified match; architecture + memory system published |
| [Maersk](maersk.md) | Container-shipping giant with human-in-the-loop GenAI over quoting and support | Supporting | Public (CPH: MAERSK), family-controlled | Claims >$300M annual savings from AI forecasting/routing (self-reported) |
...
| [Two Sigma Investments](two-sigma-investments.md) | Quant fund embedding AI as an operating layer across research/engineering | Supporting | Private, ~$60B AUM, not VC-funded | Internal tools becoming "company aware" of its own platforms and incident processes |
...
| [YouTube](youtube.md) | Google's video platform rebuilt around Gemini agents | Supporting | Alphabet subsidiary (acquired 2006, $1.65B) | Jan 2026 Gemini recommendation rewrite — biggest ranking change since watch-time (2012) |
...
**Folder vs. expectations:** all 20 expected companies are present. One profile beyond the expected list — **SonderMind** — is filed here and fits the category definition (adopter, not vendor), so treat it as a late addition rather than a misfile. Nothing expected is missing.
...
- [CaMeLs Can Use Computers Too](../../papers/tool-use-governance/camels-can-use-computers-too-system-level-security-for-comp.md) and [ClawLess: A Security Model of AI Agents](../../papers/tool-use-governance/clawless-a-security-model-of-ai-agents.md) — what "write-capable agents on production systems" (Automattic, Adobe, Figma canvas writes) should worry about: prompt injection and below-the-agent enforcement.
