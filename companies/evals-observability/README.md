...
3. **AI SREs — agents as the product** (Resolve.ai, Cleric, Traversal, Foam, Firetiger): agents that investigate production incidents autonomously, rather than tools you point at your own agents.
4. **Human data & expert feedback** (Surge AI, Snorkel, Prolific, G2i, Taste Labs): the supply side, meaning the labels, rubrics, RL environments, and preference data that evals and post-training are made of.
...
A recurring pattern worth noticing: the money is converging from both directions. Datadog invested in both Arize and Braintrust, and observability incumbents are building eval features while eval startups add monitoring. Meanwhile the AI-SRE startups are effectively observability *consumers* competing with their own data sources' native agents (Datadog Bits AI, Dynatrace Intelligence Agents).

## Start here

- **[Braintrust](braintrust.md)**: the purest expression of eval-first engineering culture ("evals as unit tests"); $800M valuation, and a customer list (Notion, Stripe, Vercel, Zapier) that doubles as a who's-who of AI product teams.
...
| [Comet](comet.md) | MLOps veteran whose center of gravity is now Opik, Apache-2.0 OSS LLM/agent evals + tracing | Silver | ~$70M, nothing raised since 2021 | Opik at ~20.3k GitHub stars, the biggest OSS footprint in the category; Cost Intelligence tracks Claude Code/Codex spend |
...
| [Dynatrace](dynatrace.md) | APM heavyweight betting agent observability is tracing with new span types; OSS dt-evals judges live `gen_ai.*` spans | Gold | Public (DT); $2B+ ARR | Hosted remote MCP server lets Claude/Copilot query production telemetry with governance, no self-hosting required |
| [Sentry](sentry.md) | Developer-default error tracking extended to agent monitoring, plus Seer, an AI debugging agent | Gold | ~$217M; >$3B valuation (2022) | 44k+ GitHub stars; its MCP server feeds real stack traces *to* coding agents, so observability flows in both directions |
...
| [GrowthBook](growthbook.md) | Open-source, warehouse-native feature flags + A/B testing; experiment stats are computed in your own warehouse | Silver | ~$23.1M (Series A, June 2025) | Online eval infrastructure in disguise: A/B test prompts, models, or agent policies without a new data pipeline |
...
| Company | One-liner | Tier | Funding posture | Standout fact |
|---|---|---|---|---|
| [Surge AI](surge-ai.md) | Expert-annotator marketplace for RLHF, rubrics, and RL environments: the labor market that trains frontier models | Supporting | Bootstrapped and profitable; 2025 raise reported at up to $1B at $15–25B+ | Reported $1.2B revenue in 2024 from ~12 frontier labs, with zero outside capital through 2024 |
...
| Company | One-liner | Tier | Funding posture | Standout fact |
|---|---|---|---|---|
| [Fiddler](fiddler.md) | Enterprise AI observability + guardrails ("AI control plane") for banks, insurers, healthcare, defense | Silver | ~$100M; Series C $30M (Jan 2026) | In-house Trust Models run guardrails in your VPC at claimed <100ms, with no per-call judge-LLM fees or data exfiltration |
...
**Agentic-SWE evaluation.** The intellectual backbone of the eval platforms; Arize, Braintrust, Comet, W&B, and Snorkel all cite this problem shape:
...
- [The Evolution of Tool Use in LLM Agents](../../papers/tool-use-governance/the-evolution-of-tool-use-in-llm-agents-from-single-tool-ca.md): multi-tool orchestration, which Resolve.ai's 60+ integrations and every tracing SDK here capture.
...
1. **Does eval tooling survive as an independent layer?** Model providers, agent frameworks, and hyperscalers are all absorbing evals and guardrails (AWS/Google bundle guardrails; frameworks ship eval harnesses). The investors who priced Braintrust at $800M and funded Arize to $131M evidently think yes; Datadog, by investing in both, is hedging as an incumbent.
...
5. **Human data at frontier prices.** Surge's reported $1.2B revenue from ~12 labs, G2i's $200/hr engineers, and Taste Labs' "judgment as a service" all bet expert human data stays scarce; synthetic data and self-play bet the opposite. The folder's five human-data vendors have all taken the scarce side of that trade.
