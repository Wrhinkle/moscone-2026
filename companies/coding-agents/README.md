...
Autonomous or assisted software-engineering agents and AI coding products: Devin-style ticket-to-PR agents, terminal/IDE coding agents, AI code review, and the infrastructure that makes agent-written code shippable. The real-world problem is twofold. First, most engineering work is delegable grunt work (migrations, bug backlogs, test coverage, dependency bumps) that agents can now do end-to-end. Second, once agents generate code at volume, the bottleneck moves downstream to review, context, and governance: someone has to verify that a torrent of machine-written PRs is actually correct, on-convention, and safe to merge. This category holds both sides of that trade, the companies generating the code and the companies checking it.

## How to read this category

The load-bearing entries are **[Cognition](cognition.md)** (Devin, the reference case for autonomous ticket-to-PR agents, at a $26B valuation and ~$492M reported ARR), **[GitHub](github.md)** (the substrate everything else plugs into, now betting on Agent HQ as the multi-vendor agent control plane), and **[OpenHands](openhands.md)** (the open-source reference implementation that most agentic-SWE benchmarks actually run). Those three mark the poles of the category: closed frontier autonomy, platform gravity, and open infrastructure.
...
- **Review / verification**: the counterweight that monetizes the AI-slop problem. [Qodo](qodo.md) (the category's platinum sponsor), [Greptile](greptile.md), [Baz](baz.md).
- **Context / grounding**: feeding agents what they need to avoid hallucinating. [Sourcegraph](sourcegraph.md) (code graph), [Tessl](tessl.md) (versioned, evaluated skills), [IMPECCABLE](impeccable.md) (design rules for frontend output).
...
1. **[cognition.md](cognition.md)**: the company that defined the category with Devin, and also the cautionary tale (13.86% SWE-bench demo → Answer.AI's 3-of-20 real-task result → $492M ARR anyway).
...
3. **[openhands.md](openhands.md)**: if you're building rather than buying, this is the open agent loop, SDK, and eval harness the research community standardized on.
...
| [Kimchi](kimchi.md) | Open-source terminal agent with multi-model routing: cheap open-weight models for bulk work, frontier only when needed | Gold | Cast AI capital ($108M Series C; >$1B valuation) | A Cast AI product line: token-cost arbitrage applied to coding agents, with a claimed 12x cost reduction |
| [Cognition](cognition.md) | Devin, the autonomous AI software engineer, plus the Windsurf IDE and its own SWE-1.x model family | Silver | ~$1.6B+ raised; $26B post (May 2026) | Goldman Sachs' CIO called Devin "our new employee"; Cognition claims 89% of its own commits are Devin-written |
...
| [Conductor (Orkes)](conductor.md) | Managed durable-execution engine (Netflix's Conductor OSS) repositioned as the runtime for production agents | Supporting | ~$89M+; $60M Series B (Apr 2026) | Closer to agent orchestration than to coding agents; the 32k-star OSS engine runs in production at Netflix, Tesla, LinkedIn |
...
| [Morph](morph.md) | "Fast Apply" inference model that merges LLM-proposed code edits into files at ~10,500 tok/s | Supporting | ~$5.75M (YC S23, as Autoinfra) | Sells the unglamorous bottleneck of edit application as an API; cites JetBrains, Vercel, Webflow as users |
...
3. **Vendor metrics all the way down.** Nearly every ROI number in this folder (Monday.com's 73.8% acceptance, Factory's 31x, Cognition's 89% dogfooding, Kimchi's 12x) is vendor-published. The Answer.AI Devin evaluation remains one of the only independent audits, and the category badly needs more.
...
5. **Context as the moat.** Sourcegraph, Tessl, and Greptile all argue that context (code graph, evaluated skills, learned team standards) outlasts the agent as the durable asset. If skills/context converge on open formats (MCP, plain markdown), the moat thins to evals and governance.
