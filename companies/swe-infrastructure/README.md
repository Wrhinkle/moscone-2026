...
CI/CD, merge queues, sandboxes and dev environments, code quality & security scanning, code intelligence, editors/terminals, and engineering analytics. This is the plumbing of software delivery — the layer that decides whether code builds, tests, merges, and ships. The real-world problem it exists to solve has changed shape: for a decade these tools removed friction for *human* developers; as of 2025–26 nearly every company in this folder has repositioned around a single thesis — **when coding agents write most of the code, deterministic verification, sandboxed execution, and governed delivery pipelines become the scarce layer**. The category is where "AI writes the code" collides with "someone still has to trust, run, and land it."
...
3. **[coder.md](coder.md)** — the enterprise/governance counterweight: egress firewalls, audited tool calls, and air-gapped agent fleets, with the memorable KKR investor-as-customer datapoint.
4. **[entire-inc.md](entire-inc.md)** — the newest and loudest signal about where the category is going: GitHub's ex-CEO raising a record $60M seed to build provenance/audit infrastructure for agent-generated commits.
...
**Tool use, governance, and security — policy enforced by the environment, not the prompt:**
...
5. **Execution risk at the incumbents and the newborns alike.** CircleCI hasn't raised since 2021 and carries a 2023 breach; Buildkite's founder-CEO exited without explanation in 2025; Daytona closed-sourced its core; Programma Labs has no public product at all. The category's repositioning stories are ahead of their proof.
