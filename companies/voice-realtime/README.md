...
The most load-bearing profiles are **[LiveKit](./livekit.md)** and **[Daily](./daily.md)** — the two open-source frameworks (LiveKit Agents, Pipecat) that nearly everything else in the category integrates with — and **[Twilio](./twilio.md)**, the PSTN on-ramp that even competing platforms pay for. The second big fault line is **compose vs. buy**: assemble Twilio/Telnyx + Deepgram/AssemblyAI + LiveKit/Pipecat yourself, or pay Vapi (or Telnyx's all-in agent, or Deepgram's Voice Agent API) $0.05/min-ish to own the pipeline for you. Watch for vertical integration in both directions: telcos adding hosted agents, model vendors adding orchestration, and speech-to-speech models threatening to collapse the cascaded pipeline everyone here monetizes.
...
| [LemonSlice](./lemonslice.md) | Turns a single still image into a live, talking two-way video call | Supporting | $10.5M seed (Matrix, YC) | One headshot or illustration becomes a real-time conversational avatar — no footage capture step |
...
| [Reactor](./reactor.md) | SDK/API for real-time generative video / interactive "world models" | Supporting | $59M Series A (Lightspeed, May 2026) | Ex-Apple Vision Pro founders; near-zero time-to-first-frame claim — but the AIEWF sponsor link (reactorhq.com) is a parked domain, so identification is Reported, not confirmed |
...
- [Adaptive Turn-Taking for Real-Time Multi-Party Voice Agents](../../papers/voice-realtime/adaptive-turn-taking-for-real-time-multi-party-voice-agents.md) — "is it my turn?" as a model decision, not a VAD heuristic; the research counterpart of Deepgram Flux, LiveKit's turn detector, and Twilio's barge-in handling.
...
1. **Does speech-to-speech collapse the cascade?** Nearly every business model here monetizes a layer of the STT→LLM→TTS pipeline. If end-to-end speech models (OpenAI Realtime, Moshi-lineage) reach production quality, standalone STT/TTS vendors and pipeline orchestrators both get squeezed — Gradium is the hedge that owns the research either way.
2. **Who owns the agent layer vs. the plumbing?** Twilio, Telnyx, Deepgram, and LiveKit are all climbing up-stack toward Vapi's territory while Vapi's providers are its suppliers. Multiple profiles flag the same risk: the platform you build on today competes with you (or your vendor) tomorrow.
3. **Do vendor latency claims survive third-party measurement?** Telnyx's sub-200ms, Vapi's sub-600ms, HeyGen's sub-second avatar claims are all vendor-reported. τ-Voice/ProVoice-Bench-style independent benchmarks for commercial stacks barely exist yet.
