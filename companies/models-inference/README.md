...
- [cerebras.md](cerebras.md): why decode speed is the new battleground for serial agent loops, with a $5.55B IPO from investors who bought that thesis.
...
| [Google DeepMind](google-deepmind.md) | Alphabet's frontier lab: Gemini, AlphaFold, Antigravity | labs | Alphabet-funded | ~8.5M developers building on Gemini monthly; killed Gemini CLI with ~30 days' notice, an adoption *and* churn warning |
...
| [Not Diamond](not-diamond.md) | Per-request meta-model that predicts which LLM will answer best/cheapest | supporting | $2.3M seed + undisclosed follow-on (SAP, IBM) | Middleware rather than a gateway: it never sees your payloads; SAP built a prompt-optimization service on it |
...
| Company | One-liner | Tier | Funding posture | Standout fact |
|---|---|---|---|---|
| [NVIDIA](nvidia.md) | The substrate under everything, plus Dynamo serving, NIM containers, and Nemotron open models | supporting | Public; ~$5T market cap, FY2026 revenue $215.9B | Now a major investor in its own customers (up to $30B into OpenAI); the circularity is a live concern among the analysts covering it |
...
| [Unsloth](unsloth.md) | Two brothers' OSS library: ~2x faster fine-tuning at 70% less VRAM, plus the default GGUF quants | supporting | ~$500K; its reach comes from open source rather than capital | 67.8K GitHub stars and 10M+ monthly Hugging Face downloads on roughly a seed round |
...
| [AIDAChip](aidachip.md) | AI copilots with shared team context for semiconductor design | supporting | Undisclosed | Flagged in its own profile as a category misfit; it is EDA tooling rather than model infrastructure |
...
6. **Circular financing.** NVIDIA invests in OpenAI, Baseten, Fireworks, and Together while selling them all GPUs; OpenAI anchors Cerebras' order book. Nobody has published a credible answer to how much category revenue is real end-demand versus capital recycling.
