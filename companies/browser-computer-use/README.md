...
Browser infrastructure for agents, computer-use agents, agent-native browsers and personal AI devices. This is the layer where agents act on the world: clicking through websites that will never ship an API and driving desktop apps by screen and keyboard, sometimes from inside the browser or device the human already uses. The category exists because the overwhelming majority of real-world software (vendor portals, legacy GUIs, checkout flows, government sites) is only reachable the way a person reaches it. Every serious agent product eventually needs a substrate to do that reliably at scale: a headless browser fleet, a sandboxed VM, or an agentic browser.

## How to read this category

The load-bearing entries are **Browserbase** (the headless infrastructure end: managed browser fleets plus the widely adopted Stagehand SDK; the category's only platinum sponsor and its clearest commercial success), **Cua** (the sandbox/eval end: "Docker for computer-use agents," full-OS VMs for training and benchmarking screen-acting models), and **Yutori** (the model end, the rare vertically integrated player that trains its own pixels-to-actions foundation model in-house). Read those three and you have the stack: infrastructure → environment → action model.
...
| [Cua](cua.md) | Open-source sandboxed full-OS VMs + benchmarking for computer-use agents | Supporting | ~$500K (YC X25), the earliest stage of the group | Founders built Microsoft's Windows Agent Arena; ~19.4k GitHub stars on ~$500K of funding |
...
- [Verifiable Software Worlds for Computer-Use Agents](../../papers/tool-use-governance/verifiable-software-worlds-for-computer-use-agents.md): reproducible, checkable environments. Cua's snapshot-native sandboxes provide exactly this, and Yutori's RL-on-live-websites training sits in the live-vs-simulated tension the paper describes.
